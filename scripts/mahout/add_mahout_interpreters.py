...
parser.add_argument("--overwrite_existing",
                    help="if %sparkMahout or %flinkMahout exist, delete them and create new ones. Otherwise Fail.",
                    action="store_true")
...
class ZeppelinTerpWrangler:
    ...
    def createTerp(self, original_terp_name, new_terp_name, overwrite_existing=True):
        ...
        if self._terpExists(new_terp_name):
            print
            "Found existing '%s' interpreter..." % new_terp_name
            if overwrite_existing:
                print
                "deleting %s from interpreter.json" % new_terp_name
        ...
        print
        "created new interpreter '%s' from interpreter '%s" % (new_terp_name, original_terp_name)
    ...
    def _addTerpDep(self, terpName="", dep="", exclusions=None):
        if self.interpreter_json == {}:
            print
            "no interpreter.json loaded, reading last one downloaded"
            self._readTerpJson()
        terp_id = self._getTerpID(terpName)
    ...
    def addMahoutConfig(self, terpName, mahout_home, mahout_version="0.12.2"):
        ...
        print
        "updating '%s' with Apache Mahout dependencies and settings" % terpName
...
if args.zeppelin_home == None:
    ...
    if (zeppelin_home.split("/")[-1] == "bin") and (isfile("zeppelin-daemon.sh")):
        print
        "we're in the zeppelin/bin"
        zeppelin_home = "/".join(zeppelin_home.split("/")[:-1])
    print
    "--zeppelin_home not specified, using %s" % zeppelin_home
else:
    zeppelin_home = args.zeppelin_home

if not valid_zeppelin_home(zeppelin_home):
    print
    "%s does not appear to be a valid ZEPPELIN_HOME - e.g. the top level directory of the ZEPPELIN install" % zeppelin_home
    exit(1)
else:
    print
...
if not isfile(interpreter_json_path):
    print
    "interpreter.json doesn't exist. Checking weather Zeppelin is running."
    status = call(["bin/zeppelin-daemon.sh", 'status'], cwd=zeppelin_home)
    if status == 1:
        print
        "Zeppelin doesn't appear to be running- it is possible that Zeppelin has never been run (interpreter.json is created when Zeppelin is run)"
        print
        "I'm going to try to start Zeppelin to create interpreter.json"
        ...
        print
        "We're in the correct top-level directory, Zeppelin appears to be running, but there is no 'interpreter.json'. \
...
if isfile(interpreter_json_path):
    ...
    print
    "'interpreter.json' not found in %s/conf" % args.zeppelin_home
    exit(1)
...
def download_mahout():
    if args.force_download:
        print
        "--force_download: OK, deleting existing tar if it exists."
        call(["rm", "%s/%s" % (zeppelin_home, tar_name)])
        return True
    elif isfile("%s/%s" % (zeppelin_home, tar_name)):
        print
        "%s found, skipping download" % tar_name
        return False
    elif args.mahout_home:
        print
        "--mahout_home set, skipping download"
        return False
    else:
        return True
...
if isfile(zeppelin_env_sh_path):
    ...
    if any(["export MAHOUT_HOME=" in line for line in zeppelin_env_sh]):
        ...
        print
        "appending '%s' to conf/zeppelin-env.sh" % mahout_home_str
...
if not args.restart_later:
    ...
    print
    "--restart_later flag detected: remember to restart Zeppelin to see new Mahout interpreters!!"
